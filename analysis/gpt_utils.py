...
def generate_feedback_from_keypoints(summary_text: str, exercise_name: str, body_part: str) -> dict:
    ...
    **평가 규칙**:
    - 각 관절에 대해 중요한 부위일수록 점수 차감이 큽니다. 예를 들어 어깨, 무릎, 척추와 같은 중요한 부위에 문제가 있으면 점수를 크게 차감합니다.
    - 문제 관절 개수에 따라 차감 점수를 늘려주세요. 예를 들어, 문제 관절이 1개일 경우 10점을 차감, 2개일 경우 20점을 차감 등으로 설정해주세요.
    - 문제 관절의 중요도에 따라 점수를 추가로 차감해주세요. 예를 들어 어깨나 척추 문제는 10점 이상 차감, 발목 문제는 5점 차감.
    - 최종 점수는 0~100 사이로 설정하되, 문제가 적은 경우 높은 점수를, 많을 경우 낮은 점수를 부여하세요.
